...
import { fetchRole, updateRole } from "../../../redux/reducers/role";
...
function UpdateRolePage() {
    ...
    useEffect(() => {
        const index = data.findIndex((value) => value.id === roleId);
        if (data[index] && data[index].id) {
            setRole(data[index]);
        }
    }, [dispatch, data, roleId]);
    ...
    useEffect(() => {
        const breadcrumbItems: BreadcrumbItem[] = [
            ...
            { label: "Quản lý vai trò", url: "/settingSystem/roleManage" },
            {
                ...
                url: "/settingSystem/roleManage/updateRole",
            },
        ];
    ...
    ///add data
    const handleUpdateRole = (type: NotificationType) => {
        if (typeNoti === "success") {
        ...
        <div className="wrapper-addRole">
            ...
            <div className="wrapper-addService__btn">
                ...
                <CustomButton
                    type="BtnDefault"
                    text="Cập nhật"
                    onClick={() => handleUpdateRole(typeNoti)}
                />
            </div>
        </div>
    );
